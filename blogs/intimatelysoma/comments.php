...
<div id="comments" class="gdlr-comments-area">
  ...
  $args = array(
      ...
      'fields' => apply_filters('comment_form_default_fields', array(
          'author' =>
              '<div class="row row-tight"><div class="col-sm-6"><div class="form-group"><label for="name" class="sr-only">Name *</label>' .
              '<input id="name" name="author" type="text" class="form-control" placeholder="Name *" value="' . esc_attr( $commenter['comment_author'] ) .
              '" data-default="' . esc_attr(__('Name*', 'gdlr_translate')) . '" size="30"' . $aria_req . ' /></div></div>',
          'email' =>
              '<div class="col-sm-6"><div class="form-group"><label for="email" class="sr-only">Email *</label>' .
              '<input id="email" name="email" type="email" class="form-control" placeholder="Email *" value="' . esc_attr(  $commenter['comment_author_email'] ) .
              '" data-default="' . esc_attr(__('Email*', 'gdlr_translate')) . '" size="30"' . $aria_req . ' /></div></div>',
//          'url' =>
//              '<div class="col-sm-4"><div class="form-group"><label for="url" class="sr-only">Website</label>' .
//              '<input id="url" name="url" type="text" class="form-control" placeholder="Website" value="' . esc_attr($commenter['comment_author_url']) .
//              '" data-default="' . esc_attr(__('Website', 'gdlr_translate')) . '" size="30" /></div></div></div>'
      )),
      'comment_field' =>  '<label for="comment" class="sr-only">Comments</label>' .
                          '<textarea class="form-control" id="comment" name="comment" cols="45" rows="6" aria-required="true">' .
                          '</textarea>'
